(function () {
            ...
            function ensureShareHashForSermon(sermon) {
                if (!sermon || !String(sermon.audioUrl || "").trim()) {
                    return Promise.resolve("");
                }
                if (sermon._njcShareHash) {
                    return Promise.resolve(sermon._njcShareHash);
                }
                return hashSermonStatDocId(sermon.audioUrl).then(function (id) {
                    sermon._njcShareHash = id || "";
                    return sermon._njcShareHash;
                }).catch(function () {
                    sermon._njcShareHash = "";
                    return "";
                });
            }

            function buildSermonAppShareUrl(shareHashId) {
                var id = String(shareHashId || "").trim().toLowerCase();
            ...
            function buildPlainShareText(title, preview, url, playLine) {
                var lines = [];
                if (title) {
                    lines.push(title);
                }
            ...
            function parseSermonDeepLinkFromHash() {
                var h = String(window.location.hash || "");
                var q = h.indexOf("?");
                if (q < 0) {
                    return { legacyUrl: "", shareId: "" };
                }
                try {
                    var sp = new URLSearchParams(h.slice(q + 1));
                    var s = String(sp.get("s") || "").trim().toLowerCase();
                    if (/^[a-f0-9]{64}$/.test(s)) {
                        return { legacyUrl: "", shareId: s };
                    }
                    var legacy = String(sp.get("sermon") || "").trim();
                    if (legacy) {
                        return { legacyUrl: legacy, shareId: "" };
                    }
                } catch (errParse) {
                    return { legacyUrl: "", shareId: "" };
                }
            ...
            function stripSermonQueryFromHash() {
                try {
                    ...
                    var sp = new URLSearchParams(h.slice(qMark + 1));
                    if (!sp.has("sermon") && !sp.has("s")) {
                        return;
                    }
            ...
            function syncSermonShareButton() {
                ...
                if (signedIn) {
                    playerShareBtn.setAttribute(
                        "aria-label",
                        T("sermons.shareAria", "Share sermon image", latestSermonsCard)
                    );
                    playerShareBtn.title = T("sermons.shareAria", "Share sermon image", latestSermonsCard);
                } else {
                    playerShareBtn.setAttribute(
                        "aria-label",
            ...
            function wrapSermonShareText(ctx, text, maxWidth, maxLines) {
            ...
            function sermonShareCanvasToPngBlob(canvas) {
                return new Promise(function (resolve, reject) {
                    try {
            ...
            function loadSermonShareImage(url) {
                if (!/^https:\/\//i.test(url)) {
            ...
            function downloadSermonShareBlob(blob, fileName) {
                ...
                anchor.href = url;
                anchor.download = fileName || "njc-sermon.png";
            ...
            function runSermonShare() {
                ...
                function sharePayloadNow(sharePayload, url, blob, fileName) {
                            ...
                            return p.catch(function (err) {
                                ...
                                if (sharePayload && sharePayload.files && sharePayload.files.length) {
                                    ...
                                    return navigator.share(textOnly).catch(function (err2) {
                                        ...
                                        if (blob) {
                                            downloadSermonShareBlob(blob, fileName);
                                            showSermonShareFeedback("sermons.shareDownloaded", "Sermon image downloaded. Share it on WhatsApp/Instagram.");
                                            return;
                                        }
            ...
            function tryOpenSermonFromHash() {
                ...
                var link = parseSermonDeepLinkFromHash();
                var idx = -1;
                if (link.shareId) {
                    idx = allSermons.findIndex(function (item) {
                        return item && String(item._njcShareHash || "").toLowerCase() === link.shareId;
                    });
                }
                if (idx < 0 && link.legacyUrl) {
                    idx = allSermons.findIndex(function (item) {
                        return item && item.audioUrl === link.legacyUrl;
                    });
                }
                if (idx < 0) {
                    return;
            ...
            function loadSermonsData() {
                Promise.allSettled([
                    ...
                    var primeShareHashes = !allSermons.length
                        ? Promise.resolve()
                        : Promise.all(allSermons.map(function (sermon) {
                            return ensureShareHashForSermon(sermon);
                        }));

                    return primeShareHashes.then(function () {
                        sermonsLoaded = true;
